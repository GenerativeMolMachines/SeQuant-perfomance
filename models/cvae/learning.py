...
from cvae import autoencoder_model
...
for strategy in filter_strategy:
    ...
    # model init
    autoencoder = autoencoder_model(height=height, width=width, depth=depth, channels=channels, latent_dim=latent_dim,
        learning_rate=learning_rate, filter_strategy=strategy, alpha=1.0, lambda_=1.0)

    # set checkpoint
    checkpoint_filepath = f'checkpoint/checkpoint_cvae'
    ...
    with open(f'trainHistoryDict/cvae.pkl', 'wb') as file_pi:
        pickle.dump(history.history, file_pi)

    # load model learning history
    with open(f'trainHistoryDict/cvae.pkl', 'rb') as f:
